        ...
        if [ -d "$project_path" ] && [ "$(ls -A "$project_path")" ]; then
            ...
            if [ "$extraction_with" == "--without-extraction" ]; then
                for restart in "${restarts[@]}"; do
                    ...
                    timeout 7200 python robust_pomdps_rl.py --project-path "$project_path" $standard_flags $restart $extraction_with > "$log_folder/${model_name}_without_extraction_restart_${restart}.log" 2> "$log_folder/${model_name}_without_extraction_restart_${restart}.err"
                done
            else
                ...
                for geometric_flag in "${geometric_flags[@]}"; do
                    for restart in "${restarts[@]}"; do
                        ...
                        timeout 7200 python robust_pomdps_rl.py --project-path "$project_path" $standard_flags $geometric_flag $restart $extraction_with > "$log_folder/${model_name}_geometric_${geometric_flag}_restart_${restart}.log" 2> "$log_folder/${model_name}_geometric_${geometric_flag}_restart_${restart}.err"
                    done
                done
            fi
