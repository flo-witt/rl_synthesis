...
@gin.configurable
class FSCLikeNetwork(network.DistributionNetwork):
    ...
    def __init__(
        ...
        state_spec = tf.TensorSpec(
            ...
            dtype=tf.float32,
            name='discrete_state')
        super(FSCLikeNetwork, self).__init__(
            input_tensor_spec=input_tensor_spec,
        ...
        lstm_spec_size = tf.math.reduce_sum([
            lstm_encoder.state_spec[i].shape[0] for i in range(len(lstm_encoder.state_spec))
        ])

        self.discrete_to_recurrent_state = layers.Dense(
            lstm_spec_size,
            activation="tanh",
            name="discrete_to_recurrent_state"
        )

        self.continuous_to_discrete_state = layers.Dense(
            self.memory_length, activation=None,
            name="continuous_to_recurrent_state"
        )
        self.prequantization_activation = layers.Lambda(
            lambda x: 1.5 * tf.tanh(x) + 0.5 * tf.tanh(-3 * x),
            name="prequantization_activation"
        )
        self.quantization_layer = layers.Lambda(
            lambda x: tf.round(x),
            name="quantization_layer"
        )
    ...
    def pack_network_state(self, network_state):
        # Unstack podél první osy (rozdělí na list tensorů)
        # Výstup: list of 2 tensors of shape [batch_size, state_dim]
        network_state = tf.unstack(network_state, axis=0)
        
        # Cast bez for cyklu – pomocí tf.nest.map_structure
        network_state = tf.nest.map_structure(
            lambda x: tf.cast(x, tf.float32), network_state
        )

        return network_state

    def call(self, observation, step_type, network_state=(), training=False):
        network_state = self.discrete_to_recurrent_state(network_state)
        # Ensure, that the network_state has correct LSTM shape with ListWrapper[64, 64]
        batch_size = tf.shape(observation)[0]
        network_state = tf.nest.map_structure(
            lambda x: tf.reshape(x, (2, batch_size, -1)), network_state
        )
        network_state = self.pack_network_state(network_state)
        ...
        network_state = tf.reshape(network_state, (batch_size, -1))
        # Convert continuous network_state to discrete state
        network_state = self.continuous_to_discrete_state(network_state)
        # Apply prequantization activation
        network_state = self.prequantization_activation(network_state)
        # Quantize the state

        if training:
            # Add noise to the quantization layer during training
            noise = tf.random.normal(
                shape=tf.shape(network_state), mean=0.0, stddev=0.35
            )
            network_state += noise

        discrete_memory = self.quantization_layer(network_state)

        # Straight-through estimator for quantization

        network_state = tf.stop_gradient(
            discrete_memory - network_state) + network_state
