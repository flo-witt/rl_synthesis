...
def generate_latex_table_from_stats(benchmark_stats):
    ...
    for model in benchmark_stats:
        ...
        records = {}
        for entry in stats:
            if "family_performance" in entry[1]:
                # Find maximum performance in the list
                list_converted = ast.literal_eval(entry[1]["family_performance"])
                operator = max if model in ["network", "drone-2-6-1", "maze-10", "rover"] else min
                max_performance = operator(list_converted)
    ...
    pre_df[f"Goal"] = []
    for model in model_family_performance:
        is_max = model in ["network", "drone-2-6-1", "maze-10", "rover"]
