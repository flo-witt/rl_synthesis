...
def generate_latex_table_from_stats(benchmark_stats):
    ...
    for model in benchmark_stats:
        ...
        for entry in stats:
            if "family_performance" in entry[1] and entry[1]["extraction_less"]==False:
                # Find largest time in family_performance_times below 3600 and its index.
                family_performance_times = ast.literal_eval(entry[1]["family_performance_times"])
                below_3600 = [time for time in family_performance_times if time < 3600]
                if below_3600:
                    largest_below_3600 = max(below_3600)
                    index_largest = below_3600.index(largest_below_3600)
                else:
                    largest_below_3600 = None
                    index_largest = None
                ...
                max_performance = operator(list_converted[i] for i in range(len(list_converted)) if (largest_below_3600 is None or family_performance_times[i] <= largest_below_3600))
    ...
    pre_df = {"Model": []}
    pre_df["rfPG"] = [
        f"{get_rfgp_values_constants()[model]:.2f}" if model in get_rfgp_values_constants() else "N/A" for model in model_family_performance
    ]
...
if __name__ == "__main__":
    base_directory = "models_large_fam_3"
