...
models_dir="experiments_models_robust_large"
...
for seed in "${seeds[@]}"; do
    ...
    for extraction_method in "si-g" "alergia"; do
        mkdir -p "results_robust_large_4/${extraction_method}_seed${seed}"
    done
    ...
    for extraction_method in "si-g" "alergia"; do
        for model in $(ls $models_dir); do
            timeout 7200 prerequisites/venv/bin/python3 robust_pomdps_rl.py --project-path "$models_dir/$model" \
                ...
                --seed $seed \
                > "results_robust_large_4/${extraction_method}_seed${seed}/${model}_output.txt" 2>&1
