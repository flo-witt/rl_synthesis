...
def update_records(model, records, entry):
    print(entry[1]["extraction_less"])
    ...
    if use_reward(model):
        ...
        # Import numpy array of boolean flags indicating available FSC nodes
        fsc_nodes = eval(
            entry[1]["available_nodes_in_fsc"].replace("array", "np.array"))[:last_index+1]
        fsc_sizes = [np.sum(fsc_nodes[i])
                     for i in range(len(fsc_nodes)) if i <= last_index] 
    ...
    verified_values = ast.literal_eval(entry[1]["family_performance"])[:last_index+1]
    if extraction_less:
        records[f"Synthesized Worst-Case extraction_less {entry[1]['extraction_type']}"].append(verified_values)
    else:
        records[f"Synthesized Worst-Case {entry[1]['extraction_type']}"].append(verified_values)
    if not extraction_less:
        records[f"RL {entry[1]['extraction_type']}"].append(rl_values)
    else:
        records[f"RL extraction_less {entry[1]['extraction_type']}"].append(rl_values)
    if not extraction_less:
        records[f"Extracted FSC {entry[1]['extraction_type']}"].append(extracted_values)
    else:
        records[f"Extracted FSC extraction_less {entry[1]['extraction_type']}"].append(extracted_values)
    records[f"FSC Size {entry[1]['extraction_type']}"].append(fsc_sizes)


    


def generate_convergence_curves(benchmark_stats, output_directory):
    ...
    for model in benchmark_stats:
        ...
        for entry in stats:
            ...
            # continue
            update_records(model, records, entry)
        ...
        for label, values_list in records.items():
            if "FSC Size" in label:
                continue
            method = "si-g" if "si-g" in label else ("alergia" if "alergia" in label else "extraction_less")
            for i, values in enumerate(values_list):
                ...
                for iteration, value in enumerate(padded):
                    data.append({
                        ...
                        "Value": value,
                        "Method": label,
                        "FSC Size": records[f"FSC Size {method}"][i][iteration] if iteration < len(records[f"FSC Size {method}"][i]) else records[f"FSC Size {method}"][i][-1],
                    })
        df = pd.DataFrame(data)
        # Vykreslit
        plot_robust_lineplot(df, model, output_directory)
        # Plot independent figure for FSC Size of Alergia extraction method.
        plot_fsc_sizes(df, model, output_directory)
...
def generate_convergence_curves_joint(benchmark_stats, output_directory, saynt_values, saynt_times, models_to_include=["evade-n17", "intercept-16", "drone-2-8-1"]):
    ...
    for i, model in enumerate(models_included):
        if model in CONSTANTS_SINGLE_PAYNT:
            constant_value = CONSTANTS_SINGLE_PAYNT[model]
            if is_negative_single(model):
...
def generate_convergence_robust():
    # base_directory = "./experiment_gru_avoid_large"
    base_directory = "./models_complete"
...
def generate_convergence_single():
    base_directory = "./experiments_external/models_single_family_large_2"
